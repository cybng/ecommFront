...
export default function ProductList() {
  ...
  const apiData=async()=>{
    ...
    const res= await axios.post("http://localhost:5000/api/getVendorImagesById",data);
...
const deleteProduct=async(email,id)=>{
  const data={email,id}
  const res = await axios.post("http://localhost:5000/api/deleteimagelist",data)
  if(res.status===200){
    apiData();
    alert("Image Deleted");
  }
        ...
        <div className="box-body">
            ...
            {auth?.user?.role==="Admin"?allProduct?.map((dt)=>(
              ...
              <tr>
                ...
                <td>
                  <button className="btn btn-default" onClick={(e)=>deleteProduct(dt?.vendorEmail,dt?._id)}>
                    <i className="fa fa-trash"></i>
                  </button>
                  {" "} 
                  {" "}
